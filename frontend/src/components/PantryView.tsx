...
import { Plus, Loader2, Trash2, ChefHat, AlertCircle, X } from 'lucide-react'
        ...
        {showAdd && (
          <div className="mb-4 rounded-2xl border border-[#F0E6E0] bg-white p-4 shadow-sm animate-[modal-enter_0.3s_ease-out]">
            ...
            <input
              ...
              className="mb-2 w-full rounded-xl border-2 border-[#F0E6E0] bg-white px-3 py-2.5 text-sm text-[#2D2D2D] placeholder-[#C4C4C4] outline-none focus:border-[#FF6B4A] focus:shadow-lg focus:shadow-[#FF6B4A]/10 transition-all"
            />
            <div className="mb-2 flex gap-2">
              <input
                ...
                className="w-20 rounded-xl border-2 border-[#F0E6E0] bg-white px-3 py-2.5 text-sm text-[#2D2D2D] outline-none focus:border-[#FF6B4A] focus:shadow-lg focus:shadow-[#FF6B4A]/10 transition-all"
              />
              <input
                type="text"
                placeholder="Unit"
                ...
                className="w-24 rounded-xl border-2 border-[#F0E6E0] bg-white px-3 py-2.5 text-sm text-[#2D2D2D] outline-none focus:border-[#FF6B4A] focus:shadow-lg focus:shadow-[#FF6B4A]/10 transition-all"
              />
              <select
                value={newItem.category}
                onChange={e => setNewItem({ ...newItem, category: e.target.value })}
                className="flex-1 rounded-xl border-2 border-[#F0E6E0] bg-white px-3 py-2.5 text-sm text-[#2D2D2D] outline-none focus:border-[#FF6B4A] focus:shadow-lg focus:shadow-[#FF6B4A]/10 transition-all"
              >
                {CATEGORIES.map(c => (
            ...
            <div className="flex gap-2">
              <button
                onClick={handleAdd}
                disabled={adding}
                className="flex-1 flex items-center justify-center gap-2 rounded-xl bg-[#FF6B4A] py-2.5 text-sm font-semibold text-white active:scale-95 transition-all disabled:opacity-50"
              >
                {adding ? <Loader2 size={16} className="animate-spin" /> : 'Add to Pantry'}
              </button>
            </div>
          </div>
        )}
        ...
        {items.length === 0 ? (
          <div className="flex flex-col items-center py-12">
            <div className="w-20 h-20 rounded-full bg-[#FF6B4A]/10 flex items-center justify-center mb-4">
              <ChefHat size={32} className="text-[#FF6B4A]" />
            </div>
            ...
            <button
              ...
              className="mt-4 flex items-center gap-2 rounded-xl bg-[#FF6B4A] px-6 py-2.5 text-sm font-semibold text-white shadow-md shadow-[#FF6B4A]/20 active:scale-95 transition-all"
            >
              <Plus size={16} />
              Add First Item
            </button>
          ...
          <div className="space-y-2">
            {items.map((item: any, idx: number) => (
              <div
                key={item.id}
                className="flex items-center justify-between rounded-2xl border border-[#F0E6E0] bg-white p-3.5 hover:border-[#FF6B4A]/30 transition-all active:scale-[0.98]"
                style={{ animationDelay: `${idx * 30}ms` }}
              >
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-xl bg-[#FFFBF7] border border-[#F0E6E0] flex items-center justify-center text-lg">
                ...
                <button
                  ...
                  className="rounded-full p-2.5 text-[#C4C4C4] hover:bg-red-50 hover:text-red-500 transition-colors active:scale-90"
